...
class UserWorkout(models.Model):
    user_workout_id = models.BigAutoField(primary_key=True)
    user = models.ForeignKey(settings.AUTH_USER_MODEL,
                             on_delete=models.CASCADE)
    user_workout_description = models.CharField(max_length=150, null=True, blank=True)
    
    def __str__(self):
        return f'{self.user} Workout'


class Exercises(models.Model):
    exercise = models.CharField(max_length=60, null=True, blank=True)
    exercise_description = models.CharField(max_length=512, null=True, blank=True)
...
class UserWorkoutSession(models.Model):
    user_workout_id = models.ForeignKey(
        UserWorkout, on_delete=models.CASCADE, related_name='user_workout', related_query_name='user_workout', null=True, blank=True)
